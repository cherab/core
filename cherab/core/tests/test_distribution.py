...
class TestZeroDistribution(unittest.TestCase):
    ...
    def test_call_returns_zero(self):
        ...
        # iterate over a subset of inputs to avoid long execution time
        for x, y, z, vx, vy, vz in product(
            self.x[::2], self.y[::2], self.z[::2], self.vx[::2], self.vy[::2], self.vz[::2]
        ):
            result = self.distribution(x, y, z, vx, vy, vz)
            self.assertEqual(
                result, 0.0, msg="__call__() should return 0.0 at ({}, {}, {}, {}, {}, {}).".format(x, y, z, vx, vy, vz)
            )

    def test_bulk_velocity_returns_zero_vector(self):
        """Test that bulk_velocity() returns zero vector for all positions."""
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            velocity = self.distribution.bulk_velocity(x, y, z)
            self.assertAlmostEqual(
                velocity.x, 0.0, delta=1e-10, msg="bulk_velocity().x should be 0.0 at ({}, {}, {}).".format(x, y, z)
            )
            self.assertAlmostEqual(
                velocity.y, 0.0, delta=1e-10, msg="bulk_velocity().y should be 0.0 at ({}, {}, {}).".format(x, y, z)
            )
            self.assertAlmostEqual(
                velocity.z, 0.0, delta=1e-10, msg="bulk_velocity().z should be 0.0 at ({}, {}, {}).".format(x, y, z)
            )
    ...
    def test_effective_temperature_returns_zero(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            ...
            self.assertEqual(
                temperature, 0.0, msg="effective_temperature() should return 0.0 at ({}, {}, {}).".format(x, y, z)
            )
    ...
    def test_density_returns_zero(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            ...
            self.assertEqual(density, 0.0, msg="density() should return 0.0 at ({}, {}, {}).".format(x, y, z))
...
class TestGenericDistribution(unittest.TestCase):
    ...
    def test_bulk_velocity(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):
            ...
            self.assertAlmostEqual(
                result.x, expected.x, delta=1e-10, msg="bulk_velocity().x is wrong at ({}, {}, {}).".format(x, y, z)
            )
            self.assertAlmostEqual(
                result.y, expected.y, delta=1e-10, msg="bulk_velocity().y is wrong at ({}, {}, {}).".format(x, y, z)
            )
            self.assertAlmostEqual(
                result.z, expected.z, delta=1e-10, msg="bulk_velocity().z is wrong at ({}, {}, {}).".format(x, y, z)
            )
    ...
    def test_effective_temperature(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            ...
            self.assertAlmostEqual(
                result, expected, delta=1e-10, msg="effective_temperature() is wrong at ({}, {}, {}).".format(x, y, z)
            )
    ...
    def test_density(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            ...
            self.assertAlmostEqual(
                result, expected, delta=1e-10, msg="density() is wrong at ({}, {}, {}).".format(x, y, z)
            )
    ...
    def test_call(self):
        ...
        for x, y, z, vx, vy, vz in product(
            ...
            self.assertAlmostEqual(
                ...
                expected,
                delta=1e-10,
                msg="__call__() is wrong at ({}, {}, {}, {}, {}, {}).".format(x, y, z, vx, vy, vz),
            )
    ...
    def test_float_inputs(self):
        ...
        distribution = GenericDistribution(
            phase_space_density_float, density_float, temperature_float, velocity_constant
        )
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            ...
            self.assertAlmostEqual(
                distribution.effective_temperature(x, y, z),
                temperature_float,
                delta=1e-10,
        ...
        for x, y, z, vx, vy, vz in product(
            ...
            self.assertAlmostEqual(
                result,
                phase_space_density_float,
                delta=1e-10,
                msg="__call__() should return constant value at ({}, {}, {}, {}, {}, {}).".format(x, y, z, vx, vy, vz),
            )
...
class TestMaxwellian(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.velocity = lambda x, y, z: (
            1.6e5 * (1 + 0.1 * np.sin(x + 2) * np.sin(y + 2) * np.sin(z + 2)) * Vector3D(1, 2, 3).normalise()
        )  # m/s
        ...
        self.phase_space_density = lambda x, y, z, vx, vy, vz: (
            self.density(x, y, z)
            / (np.sqrt(2 * np.pi) * self.sigma(x, y, z)) ** 3
            * np.exp(-((Vector3D(vx, vy, vz) - self.velocity(x, y, z)).length ** 2) / (2 * self.sigma(x, y, z) ** 2))
        )  # s^3/m^6
    ...
    def test_bulk_velocity(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            velocity = maxwellian.bulk_velocity(x, y, z)
            expected = self.velocity(x, y, z)
            self.assertAlmostEqual(
                velocity.x,
                expected.x,
    ...
    def test_effective_temperature(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            temperature = maxwellian.effective_temperature(x, y, z)
            expected = self.temperature(x, y, z)
            self.assertAlmostEqual(
                temperature,
                expected,
    ...
    def test_density(self):
        ...
        for x, y, z in product(self.x, self.y, self.z):  # iterate over all positions
            density = maxwellian.density(x, y, z)
            expected = self.density(x, y, z)
            self.assertAlmostEqual(
                density,
                expected,
    ...
    def test_value(self):
        ...
        for x, y, z, vx, vy, vz in product(
            ...
            result = maxwellian(x, y, z, vx, vy, vz)
            expected = self.phase_space_density(x, y, z, vx, vy, vz)
            self.assertAlmostEqual(
                result,
                expected,
                delta=1e-10,
                msg="call method gives a wrong phase space density at ({}, {}, {}, {}, {}, {}).".format(
                    x, y, z, vx, vy, vz
                ),
            )
