...
cdef class UniformEnergyDensity(LaserProfile):
    ...
    and volumetric energy density of 5 J·m\\ :sup:`-3` and polarisation in the y direction is created:
...
cdef class ConstantBivariateGaussian(LaserProfile):
    ...
    def _function_changed(self):
        """Energy density should be returned in units [J/m\\ :sup:`-3`].

        Energy shape in xy plane is defined by normal distribution (integral over xy plane for
        constant z is 1). The units of such distribution are [m\\ :sup:`-2`].
        ...
        ``pulse_energy / length`` gives the units [J / m\\ :sup:`3`].
        """
...
cdef class TrivariateGaussian(LaserProfile):
    ...
    The standard deviation in z direction :math:`\sigma_z` is calculated from the pulse length :math:`\tau_\mathrm{p}`, which is the
    standard deviation of the Gaussian distributed output power of the laser within a single pulse:

    .. math::
        \sigma_z = \tau_\mathrm{p} c.

    The c stands for the speed of light in vacuum.

    .. note::
        The height of the cylinder, forming the laser beam, is given by `.laser_length` and is independent from the
        temporal length of the laser pulse given by `.pulse_length`. This gives the possibility to independently control
        the size of the laser primitive and the value of the volumetric energy density.
    ...
    def _function_changed(self):
        """
        Energy density should be returned in units [J/m\\ :sup:`3`]. The integral value of the `. _distribution`
...
cdef class GaussianBeamAxisymmetric(LaserProfile):
    ...
    .. math::
        \sigma(z) = \sigma_0 \sqrt{1 + \left(\frac{z - z_0}{z_\mathrm{R}}\right)^2}.

    The :math:`z_0` is the position of the beam focus and :math:`z_\mathrm{R}` is the Rayleigh length

    .. math::
        z_\mathrm{R} = \frac{\pi \omega_0^2 n}{\lambda_\mathrm{l}}

    where the :math:`\omega_0` is the standard deviation in the xy plane in the focal point (beam waist) and :math:`\lambda_\mathrm{l}` is the central wavelength of
    ...
    The following example shows how to create a laser with pulse energy 5 J, pulse length 10 ns and with the laser cylinder primitive
    being 2 m long with 5 cm in diameter. The the standard deviation of the beam in the focal point (waist) is 5 mm and the position of the
    waist is :math:`z=50` cm. The laser wavelength is 1060 nm.
