...
cdef class ThermalCXLine(PlasmaModel):
    ...
    .. math::
        ...
        \sum_j{n_{Z_j} \mathrm{PEC}_{\mathrm{cx}}(n_\mathrm{e}, T_\mathrm{e}, T_{Z_j})}
        f(\lambda),

    where :math:`n_{Z_\mathrm{i} + 1}` is the receiver species density,
    :math:`n_{Z_j}` is the donor species density,
    ...
    :math:`T_{Z_j}` is the donor species temperature,
