...
cdef class Interpolate3DLinear(_Interpolate3DBase):
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef inline double _evaluate(self, double px, double py, double pz, int i_x, int i_y, int i_z) except? -1e999:
        """
        Evaluate the interpolating function which is valid in the area given
        by 'i_x', 'i_y' and 'i_z' at any position ('px', 'py', 'pz').
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef inline double _extrapol_linear(self, double px, double py, double pz, int i_x, int i_y, int i_z, double nearest_px, double nearest_py, double nearest_pz) except? -1e999:
        """
        Extrapolate linearly the interpolation function valid on area given by
        'i_x', 'i_y' and 'i_z' to position ('px', 'py', 'pz').
...
cdef class Interpolate3DCubic(_Interpolate3DBase):
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cdef inline double _extrapol_quadratic(self, double px, double py, double pz, int i_x, int i_y, int i_z, double nearest_px, double nearest_py, double nearest_pz) except? -1e999:
        """
        Extrapolate quadratically the interpolation function valid on area given by
        'i_x', 'i_y' and 'i_z' to position ('px', 'py', 'pz').
